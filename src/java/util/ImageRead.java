...
public class ImageRead {
	...
	public static String read(BufferedImage bi,int ii){
    	 ImageFilter imgFliter = new ImageFilter(bi);
     	 ...
     	 File xx = ImageIOHelper.createImageBmp(ss);

    	try{
    	    FileInputStream input = new FileInputStream(xx);
            FileOutputStream output = new FileOutputStream(System.getProperty("user.dir")+"\\img\\pmo"+ii+".bmp");// 把扩展名添加到原来文件的后面
    	    int in = input.read();
    	    while (in != -1) {
    	     output.write(in);
    	     in = input.read();
    	    ...
    	    output.close();
    	    OCR ocr = new OCR();
        	String rlt = ocr.recognizeText(xx, "bmp");
