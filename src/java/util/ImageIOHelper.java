...
public class ImageIOHelper {
	...
	public static File createImage(File imageFile, String imageFormat) {
		...
		try {
			...
			BMPImageWriteParam tiffWriteParam = new BMPImageWriteParam(
					Locale.US);
			...
			tempFile = tempImageFileBmp(imageFile);
	...
	public static File createImageBmp(BufferedImage bi) {
		File tempFile = null;
		try {
			tempFile = File.createTempFile("tempImageFile", ".bmp");
			...
			tempFile = tempImageFileBmp(tempFile);
			ImageOutputStream ios = ImageIO.createImageOutputStream(tempFile);
			writer.setOutput(ios);
	...
	public static File tempImageFileBmp(File imageFile) {
		String path = imageFile.getPath();
		StringBuffer strB = new StringBuffer(path);
		strB.insert(path.lastIndexOf('.'), 0);
		return new File(strB.toString().replaceFirst("(?<=\\.)(\\w+)$", "bmp"));
	}
